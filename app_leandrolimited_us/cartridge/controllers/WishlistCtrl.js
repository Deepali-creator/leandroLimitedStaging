...
// Returns pids of products in wishlist
function getWishlistItems() {
    ...
    var wishlistProductIds = new dw.util.ArrayList();
    for( var productItem in productList.object.items ){
        wishlistProductIds.add(productList.object.items[productItem].product.ID);
}
    dw.system.Logger.warn(JSON.stringify(wishlistProductIds.toArray()));

    app.getView({
        ProductList: productList.object,
        ContinueURL: dw.web.URLUtils.https('Wishlist-WishListForm')
    }).render('account/wishlist/wishlistMinimal');

    return JSON.stringify(wishlistProductIds.toArray());
}
...
// Returns pids of items in wishlist
exports.getWishlistItems = guard.ensure(['get', 'https', 'loggedIn'], getWishlistItems, {scope: 'wishlist'});
