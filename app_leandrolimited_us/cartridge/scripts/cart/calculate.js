...
exports.calculate = function(basket) {
	...
	// =====   calculation. 					     =====
    // ===================================================
    if (empty(session.privacy.NoCall) || session.privacy.NoCall != true) {
...
function calculateTax (basket) {
	// taxes for babyliss pro are gross (accounted for in the product's price)
	var shipments = basket.getShipments().iterator();
	//var totalhst : Money = basket.totalTax.newMoney(new Decimal);
	//var totalqst : Money = basket.totalTax.newMoney(new Decimal);
	while (shipments.hasNext()) {
		...
		while (shipmentLineItems.hasNext()) {
			...
			if (_lineItem.taxClassID === TaxMgr.customRateTaxClassID) {
				...
				_lineItem.updateTax(0);
				_lineItem.setTax(new Money(0,basket.currencyCode));
			}
		}
		...
		while (shipmentLineItems2.hasNext()) {
		    ...
		    //var qsttax : Money=lineItem.taxBasis.multiply(TaxMgr.getTaxRate("QST", taxJurisdictionID));
		    //var hsttax : Money=lineItem.taxBasis.multiply(TaxMgr.getTaxRate("HST", taxJurisdictionID));
		    //lineItem.custom.taxQST=qsttax.value.toFixed(2);
		    //lineItem.custom.taxHST=hsttax.value.toFixed(2);
		    //totalqst=totalqst.add(qsttax);
		    //totalhst=totalhst.add(hsttax);
	...
	//basket.custom.taxQST=totalqst.value.toFixed(2);
	//basket.custom.taxHST=totalhst.value.toFixed(2);
}
