 ...
 * @returns {Object}
 */
ProductUtils.getAvailability = function(item, quantity, basket) {
	var hasDecimal = (isNaN(quantity) || (new Number(quantity)).toString().indexOf('.') == -1) ? false : true;
	var qty = isNaN(quantity) ? 1 : (new Number(quantity)).toFixed();
	/* product availability */
	var avm : ProductAvailabilityModel = item.availabilityModel;
	var maxQty = empty(item.custom.maxOrderQuantity)?999999:item.custom.maxOrderQuantity;
	var unit = empty(item.unit)?"":item.unit;
	...
	if(!empty(basket) && !empty(maxQty)){
		var basketQtys = basket.getAllProductQuantities();
		var itemBasketQty:Number = new Number(empty(basketQtys.get(item))?0.00:basketQtys.get(item));
		var curQty:Number = itemBasketQty + new Number(qty);
		if(curQty > maxQty){
			availability.maxQtyMsg = Resource.msgf('cart.quantityinvalid','checkout',"", maxQty);
		}	
	}
	...
	availability.inStockDefaultMsg = Resource.msg('global.instock','locale','null');
	...
	// Fractional quantities not allowed RE 1887 - Mahdi
	if (hasDecimal == true){
		availability.decimalError = Resource.msg('global.nofractionalquantities','locale',null);
	}
