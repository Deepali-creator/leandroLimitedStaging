...
module.exports = function (pdict) {
	...
	*/
	if (pdict.formfield.formId === 'phone' || pdict.formfield.formId === 'country' || pdict.formfield.formId === 'postal' || pdict.formfield.formId === 'firstname'|| pdict.formfield.formId === 'lastname' || pdict.formfield.formId === 'firstName'|| pdict.formfield.formId === 'lastName' || pdict.formfield.formId === 'friendsname' || pdict.formfield.formId === 'yourname' ||  pdict.formfield.formId === 'city') {
	...
	switch (element) {
		case 'select':
			input = '<div class="select-field-wrapper"><select class="input-select ' + fieldClass + '" id="' + id + '" name="' + name + '" ' + attributes + '>';
			...
			input += options.join('');
			input += '</select><span class="select-field__icon"><i class="fa fa-caret-down"></i></span></div>';
			break;
		case 'input':
			var checked = '';
		...
		case 'radio':
			Object.keys(pdict.formfield.options).forEach(function (optionKey) {
				...
				options.push('<input class="input-radio "' + fieldClass + ' type="radio"' + checked + ' id="' + id + optionKey + '" name="' + name + '" value="' + value + '" ' + attributes + '/><label for="' + id + optionKey + '">' + Resource.msg(option.label, 'forms', null) + '</label>');
			});
			input += options.join('');
			break;
	}
